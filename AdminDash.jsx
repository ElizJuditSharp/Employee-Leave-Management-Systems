...
export default function AdminDash() {
  ...
  const takeAction = async (action) => {
    try {
      await axios.post(`${API}/api/action/${modal.id}`, { status: action, note })
  ...
  return (
    <div style={pg}>
        ...
        <div style={{ display:'flex', gap:'8px', marginBottom:'16px' }}>
          {[['all','All Requests'], ['pending','⏳ Pending Only']].map(([k, lbl]) => (
            <button key={k} onClick={() => setTab(k)} style={{
              ...
              fontWeight:700, fontSize:'13px', fontFamily:"'Nunito',Arial,sans-serif",
              background: tab === k ? '#2d6a9f' : '#fff',
              color:      tab === k ? '#fff'    : '#555',
              boxShadow:'0 2px 6px rgba(0,0,0,0.08)'
            }}>
              {lbl}
              {k === 'pending' && count('pending') > 0 &&
                <span style={{ background:'#e74c3c', color:'#fff', borderRadius:'20px', padding:'1px 7px', fontSize:'11px', marginLeft:'6px' }}>
                  {count('pending')}
                </span>
              }
            </button>
        ...
        <div style={tableCard}>
          ...
          {loading
            ...
            : <table style={{ width:'100%', borderCollapse:'collapse' }}>
                <thead>
                  <tr>
                    {['Employee','Department','Type','From','To','Days','Reason','Status','Action'].map(h => (
                ...
                <tbody>
                  {shown.map(l => (
                    <tr key={l.id}>
                      {/* Clickable employee name → goes to profile */}
                      <td style={td}>
                        <span
                          onClick={() => navigate(`/admin/employee/${l.user_id}`)}
                          style={{ color:'#2d6a9f', fontWeight:700, cursor:'pointer', textDecoration:'underline' }}
      ...
      {modal && (
        <div style={overlay}>
          <div style={modalBox}>
            <h3 style={{ color:'#1e3a5f', marginBottom:'4px', fontSize:'18px' }}>Review Leave Request</h3>
            <p style={{ color:'#888', fontSize:'13px', marginBottom:'16px' }}>
              by {modal.emp_name} · {modal.department}
            </p>

            <div style={detailBox}>
              {[
                ['Leave Type', modal.leave_type],
                ['From',       modal.from_date],
                ['To',         modal.to_date],
                ['Duration',   modal.days + ' day(s)'],
                ['Reason',     modal.reason],
              ].map(([k, v]) => (
